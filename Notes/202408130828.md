...
**Corolário 1**: Se existir ma bijeção $f: I_m \rightarrow I_n$ então $m = n$. Consequentemente, se existirem duas bijeções $\psi : I_n \rightarrow X$ e $\varphi : I_m \rightarrow X$, deve-se ter $m=n$.

**Corolário 2**: não pode haver uma bijeção $f: X \rightarrow Y$ de um conjunto finito $X$ sobre uma parte própria $Y \subset X$.
