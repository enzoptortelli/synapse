...
**Definição**: (Inversa à direita) Dadas as funções $f: A \rightarrow B$ e $g: B \rightarrow A$, dizemos que $g$ é uma inversa à direita para $f$ quando $f \circ g = id_A: A \rightarrow A$, ou seja, quando $g(f(x)) = x$ para todo $x \in A$. 
