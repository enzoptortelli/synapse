...
* Manipulação de funções, utilizando a sobrejetividade.


**Teorema 1**: Se $A$ é um subconjunto próprio de $I_n$, não pode existir uma bijeção $f: A \rightarrow I_n$.
* Absurdo, utilizando que um valor é o menor número para qual a propriedade vale.


**Corolário 1.2**: Seja $X$ um conjunto finito. Uma aplicação $f: X \rightarrow X$ é injetiva se, e somente se, é sobrejetiva.
Utilizando MDF 2, precisamos agora provar o enunciado para uma função $f: I_n \rightarrow I_n$. Assumindo injetividade, ele monta um conjunto A ($A = f(I_n)$) tal que a restrição $f: I_n \rightarrow A$ é bijetora, 
