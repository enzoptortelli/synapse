...
**Definição**: (Soma parcial de uma série) Dada uma sequência $(a_n)$ de números reais, a partir dela formamos uma nova sequência $(s_n)$, onde $s_1 = a_1, \quad s_2 = a_1 + a_2, \quad s_n = a_1 + \dots + a_n$. Assim, os números $s_n$ são as somas parciais da séries $\sum a_n$.

**Definição**: (Série convergente/divergente) Se existir o limite $s = \lim s_n$, diremos que a série $\sum a_n$ é convergente e $s = \sum a_n = \sum_{n = 1}^{\infty} = a_1 + \dots + a_n + \dots$ será a soma da série. Se $\lim s_n$ não existir, diremos que $\sum a_n$ é uma série divergente.

**Observação**: Às vezes, é conveniente considerar séries que começam no $a_0$ ao invés de $a_1$ ($\sum_{n = 0}^{\infty}a_n$).

**Observação**: A notação $\sum a_n < + \infty$ quer dizer que a séries $\sum a_n$, com $a_n \geq 0$, converge.

**Extra**: Seja $a_n \geq 0$ para todo $n \in \mathbb{N}$ e $(a'_n)$ uma subsequência de $(a_n)$. Assim, se $\sum a_n < + \infty \Rightarrow \sum a'_n < + \infty$. 
