...
**Definição**: (Fecho de um conjunto) Chama-se de fecho de um conjunto $X$ ao conjunto $\bar{X}$ formado por todos os pontos aderentes a $X$. 

**Extra**: Tem-se que $X \subset \bar{X}$, e que se $X \subset Y$, então $\bar{X} \subset \bar{Y}$.

**Definição**: (Conjunto fechado) Diz-se que um conjunto $X$ é fechado quando $X = \bar{X}$; ou seja, quando todo ponto aderente a $X$ pertence a $X$.

**Definição**: (Conjunto denso) Seja $X \subset Y$. Diz-se que $X$ é denso em $Y$ quando $Y \subset \bar{X}$; ou seja, quando todo $b \in Y$ é aderente a $X$ (por exemplo $\mathbb{Q}$ é denso em $\mathbb{R}$).
...
**Corolário**: O fecho de qualquer conjunto é um conjunto fechado. Ou seja, $\bar{\bar{X}} = \bar{X}$ para todo $X \subset \mathbb{R}$.
