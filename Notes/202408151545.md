...
**Corolário 2**: Sejam $\lim_{x \rightarrow a} f(x) = L$ e $\lim_{x \rightarrow a} g(x) = M$. Se $f(x) \leq g(x)$ para todo $x \in X - \{a\}$, então $L \leq M$.

**Teorema 2**: (Teorema do sanduíche) Sejam $f, g, h: X \rightarrow \mathbb{R}$, $a \in X'$ e $\lim_{x \rightarrow a} f(x) =  \lim_{x \rightarrow a} g(x) = L$.
# Reference
