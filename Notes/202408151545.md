...
**Definição**: (Cobertura aberta) Dizemos que uma cobertura $\cup_{\lambda \in L} C_\lambda$ quando todos os conjuntos $C_\lambda$ da união são abertos.

**Definição**: (Cobertura finita) Dizemos que uma cobertura $\cup_{\lambda \in L} C_\lambda$ é finita quando $L = \{\lambda_1, \lambda_2, \dots, \lambda_n\}$ é um conjunto finito.

**Definição**: (Subcobertura) Seja $\cup_{\lambda \in L} C_\lambda$ uma cobertura de $X$. Se $L' \subset L$ é tal que $X \subset \cup_{\lambda \in L'} C_\lambda$, dizemos que 
