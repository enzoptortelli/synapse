...
**Observação**: Nega que se tem $lim_{x \rightarrow a} f(x) = L$ equivale dizer que existe um número $\varepsilon > 0$ com a seguinte propriedade: seja qual for $\delta > 0$, pode-se sempre achar $x_\delta \in X$ tal que $0 < |x_\delta - a| < \delta$ e $|f(x_\delta) - L| \geq \varepsilon$.
# Reference
