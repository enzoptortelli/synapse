...
Assim, tomemos a função gama e o intervalo $(0, \infty)$.  Ela é definida por $\Gamma(\alpha) = \int_0^\infty t^{\alpha - 1} e^{-t}dt$ para $\alpha > 0$. Provamos que ela é finita ([[202410220703.excalidraw]]). A função gama satisfaz a seguinte propriedade: $\Gamma(\alpha + 1) = \alpha \Gamma(\alpha) \quad \alpha > 0$  ![[202410230603.excalidraw]]
