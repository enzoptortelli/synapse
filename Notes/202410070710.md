...
**Definição**: (Família locação-escala) Seja $f(x)$ uma $fdp$. Assim, para quaisquer parâmetros $- \infty < \mu < \infty$ e $\sigma > 0$, a família de $fdp's$ $f(x) = \frac{1}{\sigma}f \left(\frac{x - \mu}{\sigma} \right)$, indexadas por $(\mu, \sigma)$, é chamada de família locação-escala com $fdp$ padrão $f(x)$; $\mu$ é chamado parâmetro de locação e $\sigma$, parâmetro de escala.
