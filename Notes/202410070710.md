...
**Gama e Qui-quadrado**: Para $p \in \mathbb{N}$, seja $X \sim Gama(\frac{p}{2}, 2)$. Assim, $$f_X(x | p) = \cfrac{x^{\frac{p}{2} - 1} e^{\frac{-x}{2}}}{\Gamma(\frac{p}{2}) 2^{\frac{p}{2}}} \mathbb{1}_{(0, \infty)}$$, que é a densidade de uma variável aleatória qui-quadrado com $p$ graus de liberdade. A esperança e a variância da qui-quadrado podem ser obtidas a partir da esperança e da variância da gama.

**Gama e Exponencial**: Seja $X \sim Gama(1, \beta)$. Assim, $$f_X(x | \beta) = \frac{e^{\frac{-x}{\beta}}}{\beta}$$
