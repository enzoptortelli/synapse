...
Entretanto, essa ainda não é a família de distribuição gama, pois essa família possui dois parâmetros. Dessa forma, podemos obter essa família definindo $X = \beta T$, onde $\beta > 0$ e $T$ é uma variável aleatória cuja $fdp$ é dada acima. Assim, teremos que $$ f_X(x | \alpha, \beta) = \cfrac{x^{\alpha - 1} e^{-\frac{x}{\beta}}}{\Gamma(\alpha) \beta^\alpha} \mathbb{1}_{(0, \infty)}$$


[[202410230622.excalidraw]]
