...
**Definição**: (Família de locação) Seja $f(x)$ uma $fdp$. Assim, a família de funções $f(x - \mu)$, indexadas pelo parâmetro $-\infty < \mu < \infty$, é chamada de família de locação com $fdp$ padrão $f(x)$ e $\mu$ é chamado de parâmetro de locação da família.

**Definição**: (Família escala)
