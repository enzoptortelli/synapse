...
**Resultado**: Se $X \sim Uniforme(0, 1)$, e $Y = - \log (X)$, então $Y \sim exp(1)$.

**Teorema**: Seja $X$ uma variável aleatória com fdp $f_X(x)$ e $Y = g(X)$, onde $g$ é uma função monótona. Suponha que $f_X$ é contínua em $\mathcal{X}$ e que $g^{-1}$ tenha derivada contínua em $\mathcal{Y}$. Então a fdp de $Y$ é dada por $$f_Y(y) = f_X(g^{-1}(y))|\
