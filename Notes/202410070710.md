...
**Teorema**: (fonte: vozes da minha cabeça) Uma variável aleatória define um novo espaço amostral $\mathcal{X} = \{a \in \mathbb{R} | \exists \quad s \in S \quad X(s) = a\}$, $X: S \rightarrow \mathcal{X}$ . Eu imagino que esse novo espaço amostral defina uma nova sigma-álgebra, $\mathcal{B}_X$, e uma nova função de probabilidade $P_X: \mathcal{B}_X \rightarrow [0, 1]$. 

**Teorema**: A função $P_X(X = x) = P(\{s \in S | X(s) = x\})$ é uma função de probabilidade
