...
Também, podemos "generalizar" (minhas palavras) a parametrização da família exponencial como $$f(x | \boldsymbol{\eta}) = h(x) c^*(\boldsymbol{\eta}) \exp \left({\sum_{i = 1}^k \eta_i t_i(x)}\right)$$, onde $h(x)$ e $t_i(x)$ são as mesmas funções que anteriormente. O conjunto $\mathcal{H} = \{\boldsymbol{\eta} = (\eta_1, \dots, \eta_k): \int_{- \infty}^{\infty} h(x) \exp({\sum_{i = 1}^k \eta_i t_i(x)}) < \infty \}$ é chamado espaço paramétrico natural da família (a integral é substituída por um somatório caso tenhamos uma $fmp$). Para $\boldsymbol{\eta} \in \mathcal{H}$, temos que ter $c^*(\boldsymbol{\eta}) = \left[\int_{- \infty}^{\infty} h(x) \exp({\sum_{i = 1}^k \eta_i t_i(x)}) < \infty \} \right]^{-1}$ para que $f$ integre $1$. 
...
Exerc interessantes 2.27 e 3.27.
