...
**Teorema**: Seja $f(\cdot)$ uma $fdp$. Seja $\mu$ qualquer número real e $\sigma$ qualquer número real positivo. Assim, $X$ é uma variável aleatória com $fdp$ $\frac{1}{\sigma}f \left(\frac{x - \mu}{\sigma} \right)$ se, e somente se, existir uma variável aleatória $Z$ com $fdp$ $f(x)$ e $X = \sigma Z + \mu$.
