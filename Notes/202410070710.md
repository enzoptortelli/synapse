...
Uma família de $fdp's$/$fmp's$ é chamada de família exponencial se podem ser expressas como: $$f(x|\mathbf{\theta}) = h(x)c(\mathbf{\theta})exp(\sum_{i = 1}^{k} w_i(\mathbf{\theta}) t_i(x))$$
